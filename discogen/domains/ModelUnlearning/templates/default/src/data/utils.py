...
def preprocess_chat_instance(
    ...
    if template_config["apply_chat_template"]:
        ...
        chat_ids = tokenizer.apply_chat_template(
            chat, tokenize=True, add_generation_prompt=False, max_length=max_length, truncation=True, **date_info
        )
        ...
        prompt_ids = tokenizer.apply_chat_template(
            chat[:-1], tokenize=True, add_generation_prompt=True, max_length=max_length, truncation=True, **date_info
        )
    else:
        wrapped_prompt = ""
...
def preprocess_pretraining_instance(
    ...
    full_seq_ids = tokenizer(
        prefix + (" " if insert_space else "") + text_content, add_special_tokens=True, max_length = max_length, truncation=True
    )["input_ids"]
    prefix_ids = tokenizer(prefix, add_special_tokens=True, max_length = max_length, truncation=True)["input_ids"]
