...
def acq_fn_optimizer(candidate_samples: jnp.ndarray,
    ...
    """
    Function to optimise the acquisition function over a set of candidate points.
    Can use local optimization to further maximise the acquisition function.
    Args:
    ...
    sorted_idxs = jnp.argsort(acq_vals, descending=True)
    sorted_candidate_samples = candidate_samples[sorted_idxs]
    sorted_acq_fn_vals = acq_vals[sorted_idxs]

    # --- return samples sorted by acquisition function values, as well as acquisition function values at these points ---
    return sorted_candidate_samples, sorted_acq_fn_vals
