"""Argoverse2 dataset loader for trajectory prediction."""
import pickle
import numpy as np
...
def download_dataset(dest_loc: str):
    """Download Argoverse2 trajectory data from HuggingFace."""
...
def load_dataset(src_loc: str = "./Argoverse2/data") -> Dict[str, Dataset]:
    """Load Argoverse2 dataset with 80/20 train/test split."""
    ...
    expected = _get_remote_count("saeedrmd/trajectory-prediction-argoverse2")
    if expected > 0 and len(pkl_files) < expected:
        print(
            ...
            "To fix: delete the cache/Argoverse2 directory and re-run create-task."
        )
