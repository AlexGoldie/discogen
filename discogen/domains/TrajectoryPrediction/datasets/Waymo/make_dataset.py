"""Waymo dataset loader for trajectory prediction."""
import pickle
import numpy as np
...
def download_dataset(dest_loc: str):
    """Download Waymo trajectory data from HuggingFace."""
...
def load_dataset(src_loc: str = "./Waymo/data") -> Dict[str, Dataset]:
    """Load Waymo dataset with 80/20 train/test split."""
    ...
    expected = _get_remote_count("saeedrmd/trajectory-prediction-waymo")
    if expected > 0 and len(pkl_files) < expected:
        print(
            ...
            "To fix: delete the cache/Waymo directory and re-run create-task."
        )
