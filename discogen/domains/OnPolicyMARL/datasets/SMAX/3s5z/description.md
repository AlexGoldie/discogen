...
OBSERVATION SPACE
The observation type is unit_list. Each agent receives a flat vector of shape (205,) built from three groups of features:
...
Total observation size: 13×(8−1) + 13×8 + 10 = 205
...
SCENARIO
3 stalkers and 5 zealots vs 3 stalkers and 5 zealots. A larger symmetric mixed-unit scenario building on the same unit types as 2s3z. Stalkers are ranged fighters (attack range 6.0, health 160, damage 13.0, cooldown 1.87 s) and zealots are melee units (attack range 2.0, health 150, damage 8.0, cooldown 0.86 s). With 8 agents per side, the coordination problem is significantly harder: more agents must independently infer the same role differentiation and focus-fire discipline from local observations alone. Larger team sizes also increase the importance of target selection, since concentrating fire on a single enemy is harder to achieve without communication. Total allied agents: 8. Observation size: 205. Actions per agent: 13.
