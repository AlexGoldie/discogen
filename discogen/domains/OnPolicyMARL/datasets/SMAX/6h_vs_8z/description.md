...
OBSERVATION SPACE
The observation type is unit_list. Each agent receives a flat vector of shape (179,) built from three groups of features:
...
Each other allied unit (13 features each, 5 entries): normalised health, relative x/y position normalised by the observing unit's sight range, previous movement vector (x, y), previous attack target index, weapon cooldown, unit type one-hot
Each enemy unit (13 features each, 8 entries): same 13-feature structure; previous actions are visible because see_enemy_actions=True

Units outside the observing unit's sight range appear with all-zero features. All values are continuous real numbers.
Total observation size: 13×(6−1) + 13×8 + 10 = 179
...
5–12: attack enemy unit 0 through 7

Total actions per ally agent: 13 (5 movement/stop + 8 attack targets). Unavailable actions (e.g. attacking a unit out of range, moving when dead) are masked each step (GET_AVAIL_ACTIONS=True).
...
SCENARIO
6 allied hydralisks vs 8 enemy zealots. A hard asymmetric scenario pairing fragile ranged allies against a large melee force. Hydralisks are ranged units (attack range 5.0, health 80, damage 12.0, cooldown 0.59 s, velocity 3.15) while zealots are melee (attack range 2.0, health 150, damage 8.0, cooldown 0.86 s, velocity 3.15). Allies are both outnumbered (6v8) and individually fragile: a hydralisk has 80 HP versus a zealot's 150, meaning any melee contact is disproportionately costly. The winning strategy requires kiting — maintaining distance from all zealots while concentrating fire to eliminate them one at a time — across 6 independent agents with no shared communication. The similar movement speeds of both unit types (3.15) make sustained kiting difficult, increasing the precision required. Total allied agents: 6. Observation size: 179. Actions per agent: 13.
