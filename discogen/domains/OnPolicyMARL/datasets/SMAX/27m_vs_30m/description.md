...
OBSERVATION SPACE
The observation type is unit_list. Each agent receives a flat vector of shape (738,) built from three groups of features:
...
Each other allied unit (13 features each, 26 entries): normalised health, relative x/y position normalised by the observing unit's sight range, previous movement vector (x, y), previous attack target index, weapon cooldown, unit type one-hot
Each enemy unit (13 features each, 30 entries): same 13-feature structure; previous actions are visible because see_enemy_actions=True

Units outside the observing unit's sight range appear with all-zero features. All values are continuous real numbers.
Total observation size: 13×(27−1) + 13×30 + 10 = 738
...
5–34: attack enemy unit 0 through 29

Total actions per ally agent: 35 (5 movement/stop + 30 attack targets). Unavailable actions (e.g. attacking a unit out of range, moving when dead) are masked each step (GET_AVAIL_ACTIONS=True).
...
SCENARIO
27 allied marines vs 30 enemy marines. The largest all-marine scenario, testing the scalability of multi-agent algorithms under severe numerical disadvantage. Marines are ranged infantry (attack range 5.0, health 45, damage 9.0, cooldown 0.61 s, sight range 9.0). With 27 simultaneously acting agents and 738-dimensional per-agent observations (most of which will be zeroed for distant units), the primary challenge is scalability: coordinating focus-fire across 27 independent agents who can each only see a local neighbourhood. Because marine health is low (45 HP), a single enemy eliminated quickly reduces incoming damage; conversely, spreading fire across 30 targets leaves all enemies alive and dealing full damage for much longer. Total allied agents: 27. Observation size: 738. Actions per agent: 35.
