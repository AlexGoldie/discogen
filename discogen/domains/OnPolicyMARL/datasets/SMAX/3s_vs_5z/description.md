...
OBSERVATION SPACE
The observation type is unit_list. Each agent receives a flat vector of shape (101,) built from three groups of features:
...
Each other allied unit (13 features each, 2 entries): normalised health, relative x/y position normalised by the observing unit's sight range, previous movement vector (x, y), previous attack target index, weapon cooldown, unit type one-hot
...
Total observation size: 13×(3−1) + 13×5 + 10 = 101
...
SCENARIO
3 allied stalkers vs 5 enemy zealots. A hard asymmetric scenario pitting a small ranged team against a larger melee force. Stalkers are ranged (attack range 6.0, health 160, damage 13.0, cooldown 1.87 s, velocity 4.13) while zealots are melee (attack range 2.0, health 150, damage 8.0, cooldown 0.86 s, velocity 3.15). The ally advantage is range: stalkers can deal damage to zealots before zealots can retaliate. The key learned behaviour is kiting — moving away from approaching zealots while simultaneously firing — so that melee contact is avoided entirely or minimised. With only 3 agents against 5 enemies, coordinated focus-fire is also essential to reduce the number of active zealots quickly. Allowing zealots to close the gap is costly because zealot cooldown is short and their melee damage accumulates fast. Total allied agents: 3. Observation size: 101. Actions per agent: 10.
