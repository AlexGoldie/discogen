...
SCENARIO
2 stalkers and 3 zealots vs 2 stalkers and 3 zealots. A symmetric mixed-unit scenario where agents must learn distinct roles for two unit types with fundamentally different combat styles. Stalkers are ranged fighters (attack range 6.0, health 160, damage 13.0 per attack, cooldown 1.87 s) while zealots are melee units (attack range 2.0, health 150, damage 8.0, cooldown 0.86 s). Stalkers should deal sustained ranged damage while zealots close the gap and absorb hits. The core challenge is coordinating across unit types without shared communication: each agent only knows its local view, so role differentiation must emerge from the policy rather than explicit assignment. Total allied agents: 5. Observation size: 127. Actions per agent: 10.
