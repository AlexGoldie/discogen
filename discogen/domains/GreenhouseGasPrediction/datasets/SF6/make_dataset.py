"""Dataset loader for MaunaLoaSF6-2015 training data."""
...
def download_dataset(dest_loc: str) -> None:
    ...
    train_source_file = Path(__file__).parent / "sf6_train.npy"
    train_dest_file = Path(dest_loc) / "train_data.npy"
    train_dest_file.parent.mkdir(parents=True, exist_ok=True)

    test_source_file = Path(__file__).parent / "sf6_test.npy"
...
def load_train_dataset(src_loc: str = "./data") -> np.ndarray:
    ...
    Args:
        src_loc: Path to the data directory (default: ./data)
                 When called from task_src/MaunaLoaSF6-2015/, this resolves to task_src/MaunaLoaSF6-2015/data

    Returns:
        numpy array with shape (N, 5) containing SF6 data from 1997-2014
    """
    data_file = Path(src_loc) / "sf6_train.npy"
...
def load_test_dataset(src_loc: str = "./data") -> np.ndarray:
    ...
    Args:
        src_loc: Path to the data directory (default: ./data)
                 When called from task_src/MaunaLoaSF6-2025/, this resolves to task_src/MaunaLoaSF6-2025/data

    Returns:
        numpy array with shape (N, 5) containing SF6 data from 2015-2025
    """
    data_file = Path(src_loc) / "sf6_test.npy"
    return np.load(data_file)
