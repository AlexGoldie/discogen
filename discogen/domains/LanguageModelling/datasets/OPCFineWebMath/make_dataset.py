...
def download_dataset(dest_loc: str):
    """
    Download and preprocess the OPC-FineWeb-Math dataset into binary files.

    This function:
    1. Downloads OPC-FineWeb-Math from HuggingFace
    ...
    output_dir = os.path.join(dest_loc, 'opcfinewebmath')
    os.makedirs(output_dir, exist_ok=True)

    print("Loading OPC-FineWeb-Math dataset from HuggingFace...")
    ...
    try:
        ...
        fs = HfFileSystem()
        all_files = fs.ls("datasets/OpenCoder-LLM/opc-fineweb-math-corpus/data", detail=False)
    ...
    # Initialize tokenizer - using GPT-2 tokenizer for consistency with FineWeb
    print("Loading tokenizer...")
    tokenizer = AutoTokenizer.from_pretrained("gpt2")

    # Process each split
    splits = [
        ("train", train_dataset, "opcfinewebmath_train_000001.bin"),
        ("validation", val_dataset, "opcfinewebmath_val_000000.bin")
    ]
...
def load_dataset(src_loc: Path = Path(__file__).parent / "data"):
    ...
    Args:
        src_loc: Path to the data directory (default: task_src/OPCFineWebMath/data)
                 When called from task_src/OPCFineWebMath/, this resolves to task_src/OPCFineWebMath/data

    Returns:
        Dictionary with "train", "val", and "vocab_size" keys
    """
    # Construct paths relative to src_loc
    # Data is stored in opcfinewebmath subdirectory
    base_path = os.path.join(src_loc, 'opcfinewebmath')

    train_pattern = os.path.join(base_path, 'opcfinewebmath_train_*.bin')
    val_pattern = os.path.join(base_path, 'opcfinewebmath_val_*.bin')
...
if __name__ == "__main__":
    ...
    dest_path = sys.argv[1] if len(sys.argv) > 1 else "./datasets/OPCFineWebMath"
